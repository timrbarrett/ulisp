...
inline void uLisp::myfree (object *obj) {
  car(obj) = NULL;
  cdr(obj) = Freelist;
  Freelist = obj;
  Freespace++;
...
object *uLisp::cons (object *arg1, object *arg2) {
  object *ptr = myalloc();
  ptr->car = arg1;
  ptr->cdr = arg2;
  return ptr;
...
object *uLisp::symbol (symbol_t name) {
  object *ptr = myalloc();
  ptr->type = SYMBOL;
  ptr->name = name;
  return ptr;
...
object *uLisp::stream (unsigned char streamtype, unsigned char address) {
  object *ptr = myalloc();
  ptr->type = STREAM;
...
void uLisp::markobject (object *obj) {
  MARK:
  if (obj == NULL) return;
...
void uLisp::gc (object *form, object *env) {
  #if defined(printgcs)
  int start = Freespace;
  #endif
  markobject(tee); 
...
void uLisp::movepointer (object *from, object *to) {
...
int uLisp::compactimage (object **arg) {
  markobject(tee);
  markobject(GlobalEnv);
  markobject(GCStack);
...
int uLisp::saveimage (object *arg) {
...
void uLisp::error2 (object *symbol, PGM_P string) {
  pfl(pserial); pfstring(PSTR("Error: "), pserial);
...
boolean uLisp::tracing (symbol_t name) {
  if (name == 0) return 0;
  int i = 0;
  while (i < TRACEMAX) {
...
void uLisp::trace (symbol_t name) {
...
void uLisp::untrace (symbol_t name) {
  int i = 0;
  while (i < TRACEMAX) {
...
boolean uLisp::consp (object *x) {
  if (x == NULL) return false;
  unsigned int type = x->type;
  return type >= PAIR || type == ZERO;
}

boolean uLisp::atom (object *x) {
  if (x == NULL) return true;
  unsigned int type = x->type;
  return type < PAIR && type != ZERO;
}

boolean uLisp::listp (object *x) {
  if (x == NULL) return true;
  unsigned int type = x->type;
  return type >= PAIR || type == ZERO;
}

int uLisp::toradix40 (char ch) {
  if (ch == 0) return 0;
...
int uLisp::integer (object *obj){
  if (!numberp(obj)) error(PSTR("Not a number"));
  return obj->integer;
}

int uLisp::fromchar (object *obj){
  if (!characterp(obj)) error(PSTR("Not a character"));
  return obj->integer;
}

int uLisp::istream (object *obj){
  if (!streamp(obj)) error(PSTR("Not a stream"));
  return obj->integer;
}

int uLisp::issymbol (object *obj, symbol_t n) {
  return symbolp(obj) && obj->name == n;
}

int uLisp::eq (object *arg1, object *arg2) {
...
int uLisp::listlength (object *list) {
  int length = 0;
  while (list != NULL) {
    list = cdr(list);
    length++;
...
object *uLisp::assoc (object *key, object *list) {
  while (list != NULL) {
...
object *uLisp::delassoc (object *key, object **alist) {
  object *list = *alist;
  object *prev = NULL;
  while (list != NULL) {
    object *pair = first(list);
...
void uLisp::indent (int spaces) {
  for (int i=0; i<spaces; i++) pserial(' ');
}

void uLisp::buildstring (char ch, int *chars, object **head) {
  static object* tail;
  if (*chars == 0) {
    *chars = ch<<8;
    object *cell = myalloc();
...
object *uLisp::readstring (char delim, gfun_t gfun) {
  object *obj = myalloc();
  obj->type = STRING;
  char ch = gfun();
  object *head = NULL;
...
int uLisp::stringlength (object *form) {
  int length = 0;
  form = cdr(form);
  while (form != NULL) {
...
char uLisp::nthchar (object *string, int n) {
...
object *uLisp::value (symbol_t n, object *env) {
  while (env != NULL) {
    object *pair = car(env);
...
object *uLisp::findvalue (object *var, object *env) {
...
object *uLisp::findtwin (object *var, object *env) {
  while (env != NULL) {
    object *pair = car(env);
...
void uLisp::dropframe (int tc, object **env) {
  if (tc) {
...
object *uLisp::closure (object *fname, object *state, object *function, object *args, object **env) {
...
object *uLisp::apply (object *function, object *args, object **env) {
  if (symbolp(function)) {
...
object **uLisp::place (object *args, object *env) {
...
inline object *uLisp::carx (object *arg) {
  if (!listp(arg)) error(PSTR("Can't take car"));
  if (arg == nil) return nil;
  return car(arg);
}

inline object *uLisp::cdrx (object *arg) {
...
void uLisp::I2Cinit(bool enablePullup) {
  TWSR = 0;                        // no prescaler
...
bool uLisp::I2Cwrite(uint8_t data) {
  TWDR = data;
  TWCR = 1<<TWINT | 1 << TWEN;
  while (!(TWCR & 1<<TWINT));
...
bool uLisp::I2Cstart(uint8_t address, uint8_t read) {
...
bool uLisp::I2Crestart(uint8_t address, uint8_t read) {
  return I2Cstart(address, read);
}

void uLisp::I2Cstop(uint8_t read) {
  (void) read;
  TWCR = 1<<TWINT | 1<<TWEN | 1<<TWSTO;
  while (TWCR & 1<<TWSTO); // wait until stop and bus released
}

inline char uLisp::spiread () { return SPI.transfer(0); }
...
gfun_t uLisp::gstreamfun (object *args) {
...
inline void uLisp::spiwrite (char c) { SPI.transfer(c); }
#if defined(__AVR_ATmega1284P__)
inline void uLisp::serial1write (char c) { Serial1.write(c); }
#elif defined(__AVR_ATmega2560__)
inline void uLisp::serial1write (char c) { Serial1.write(c); }
inline void uLisp::serial2write (char c) { Serial2.write(c); }
inline void uLisp::serial3write (char c) { Serial3.write(c); }
#endif

pfun_t uLisp::pstreamfun (object *args) {
  int streamtype = SERIALSTREAM;
  int address = 0;
  pfun_t pfun = pserial;
  if (args != NULL) {
...
void uLisp::checkanalogread (int pin) {
...
void uLisp::checkanalogwrite (int pin) {
#if defined(__AVR_ATmega328P__)
...
void uLisp::playnote (int pin, int note, int octave) {
  #if defined(__AVR_ATmega328P__)
  if (pin == 3) {
...
object *uLisp::p_quote (object *args, object *env) {
  (void) env;
  return first(args);
}

object *uLisp::sp_defun (object *args, object *env) {
  (void) env;
...
object *uLisp::sp_defvar (object *args, object *env) {
  object *var = first(args);
  if (var->type != SYMBOL) error2(var, PSTR("is not a symbol"));
...
object *uLisp::sp_setq (object *args, object *env) {
...
object *uLisp::sp_loop (object *args, object *env) {
  clrflag(RETURNFLAG);
  object *start = args;
  for (;;) {
    args = start;
...
object *uLisp::tf_if (object *args, object *env) {
  if (eval(first(args), env) != nil) return second(args);
  args = cddr(args);
  return (args != NULL) ? first(args) : nil;
}

object *uLisp::tf_cond (object *args, object *env) {
  while (args != NULL) {
...
object *uLisp::tf_when (object *args, object *env) {
  if (eval(first(args), env) != nil) return tf_progn(cdr(args),env);
  else return nil;
}

object *uLisp::tf_unless (object *args, object *env) {
  if (eval(first(args), env) != nil) return nil;
  else return tf_progn(cdr(args),env);
}

object *uLisp::tf_and (object *args, object *env) {
...
object *uLisp::tf_or (object *args, object *env) {
  object *more = cdr(args);
  while (more != NULL) {
    object *result = eval(car(args), env);
...
object *uLisp::fn_not (object *args, object *env) {
  (void) env;
  return (first(args) == nil) ? tee : nil;
}

object *uLisp::fn_cons (object *args, object *env) {
  (void) env;
  return cons(first(args),second(args));
}

object *uLisp::fn_atom (object *args, object *env) {
  (void) env;
  return atom(first(args)) ? tee : nil;
}

object *uLisp::fn_listp (object *args, object *env) {
  (void) env;
  return listp(first(args)) ? tee : nil;
}

object *uLisp::fn_consp (object *args, object *env) {
  (void) env;
  return consp(first(args)) ? tee : nil;
}

object *uLisp::fn_numberp (object *args, object *env) {
  (void) env;
  return numberp(first(args)) ? tee : nil;
}

object *uLisp::fn_symbolp (object *args, object *env) {
  (void) env;
  return symbolp(first(args)) ? tee : nil;
}

object *uLisp::fn_streamp (object *args, object *env) {
  (void) env;
  return streamp(first(args)) ? tee : nil;
}

object *uLisp::fn_eq (object *args, object *env) {
  (void) env;
...
object *uLisp::fn_car (object *args, object *env) {
  (void) env;
  return carx(first(args));
}

object *uLisp::fn_cdr (object *args, object *env) {
  (void) env;
  return cdrx(first(args));
}

object *uLisp::fn_caar (object *args, object *env) {
  (void) env;
  return carx(carx(first(args)));
}

object *uLisp::fn_cadr (object *args, object *env) {
  (void) env;
  return carx(cdrx(first(args)));
}

object *uLisp::fn_cdar (object *args, object *env) {
  (void) env;
  return cdrx(carx(first(args)));
}

object *uLisp::fn_cddr (object *args, object *env) {
  (void) env;
  return cdrx(cdrx(first(args)));
}

object *uLisp::fn_caaar (object *args, object *env) {
  (void) env;
  return carx(carx(carx(first(args))));
}

object *uLisp::fn_caadr (object *args, object *env) {
  (void) env;
  return carx(carx(cdrx(first(args))));
}

object *uLisp::fn_cadar (object *args, object *env) {
  (void) env;
  return carx(cdrx(carx(first(args))));
}

object *uLisp::fn_caddr (object *args, object *env) {
  (void) env;
  return carx(cdrx(cdrx(first(args))));
}

object *uLisp::fn_cdaar (object *args, object *env) {
  (void) env;
  return cdrx(carx(carx(first(args))));
}

object *uLisp::fn_cdadr (object *args, object *env) {
  (void) env;
  return cdrx(carx(cdrx(first(args))));
}

object *uLisp::n_cddar (object *args, object *env) {
  (void) env;
  return cdrx(cdrx(carx(first(args))));
}

object *uLisp::fn_cdddr (object *args, object *env) {
  (void) env;
  return cdrx(cdrx(cdrx(first(args))));
}

object *uLisp::fn_length (object *args, object *env) {
  (void) env;
...
object *uLisp::fn_list (object *args, object *env) {
  (void) env;
  return args;
}

object *uLisp::fn_reverse (object *args, object *env) {
  (void) env;
...
object *uLisp::fn_nth (object *args, object *env) {
  (void) env;
...
object *uLisp::fn_assoc (object *args, object *env) {
  (void) env;
...
object *uLisp::fn_member (object *args, object *env) {
  (void) env;
...
object *uLisp::fn_apply (object *args, object *env) {
...
object *uLisp::fn_funcall (object *args, object *env) {
  return apply(first(args), cdr(args), &env);
}

object *uLisp::fn_append (object *args, object *env) {
  (void) env;
  object *head = NULL;
  object *tail = NULL;
  while (args != NULL) {
...
object *uLisp::fn_mapc (object *args, object *env) {
  object *function = first(args);
...
object *uLisp::fn_mapcar (object *args, object *env) {
  object *function = first(args);
  object *list1 = second(args);
...
object *uLisp::fn_add (object *args, object *env) {
  (void) env;
  int result = 0;
  while (args != NULL) {
...
object *uLisp::fn_subtract (object *args, object *env) {
  (void) env;
...
object *uLisp::fn_multiply (object *args, object *env) {
  (void) env;
  int result = 1;
  while (args != NULL){
    #if defined(checkoverflow)
...
object *uLisp::fn_divide (object *args, object *env) {
  (void) env;
...
object *uLisp::fn_mod (object *args, object *env) {
  (void) env;
...
object *uLisp::fn_oneplus (object *args, object *env) {
  (void) env;
  int result = integer(first(args));
  #if defined(checkoverflow)
...
object *uLisp::fn_oneminus (object *args, object *env) {
  (void) env;
  int result = integer(first(args));
  #if defined(checkoverflow)
...
object *uLisp::fn_abs (object *args, object *env) {
  (void) env;
  int result = integer(first(args));
  #if defined(checkoverflow)
...
object *uLisp::fn_random (object *args, object *env) {
  (void) env;
  int arg = integer(first(args));
  return number(random(arg));
}

object *uLisp::fn_max (object *args, object *env) {
  (void) env;
  int result = integer(first(args));
  args = cdr(args);
  while (args != NULL) {
...
object *uLisp::fn_min (object *args, object *env) {
  (void) env;
  int result = integer(first(args));
  args = cdr(args);
  while (args != NULL) {
...
object *uLisp::fn_numeq (object *args, object *env) {
  (void) env;
  int arg1 = integer(first(args));
  args = cdr(args);
  while (args != NULL) {
...
object *uLisp::fn_less (object *args, object *env) {
  (void) env;
  int arg1 = integer(first(args));
  args = cdr(args);
  while (args != NULL) {
...
object *uLisp::fn_lesseq (object *args, object *env) {
  (void) env;
  int arg1 = integer(first(args));
  args = cdr(args);
  while (args != NULL) {
...
object *uLisp::fn_greater (object *args, object *env) {
  (void) env;
  int arg1 = integer(first(args));
  args = cdr(args);
  while (args != NULL) {
...
object *uLisp::fn_greatereq (object *args, object *env) {
  (void) env;
  int arg1 = integer(first(args));
  args = cdr(args);
  while (args != NULL) {
...
object *uLisp::fn_noteq (object *args, object *env) {
  (void) env;
  while (args != NULL) {   
    object *nargs = args;
...
object *uLisp::fn_plusp (object *args, object *env) {
  (void) env;
  int arg = integer(first(args));
  if (arg > 0) return tee;
  else return nil;
...
object *uLisp::fn_minusp (object *args, object *env) {
  (void) env;
  int arg = integer(first(args));
  if (arg < 0) return tee;
  else return nil;
...
object *uLisp::fn_zerop (object *args, object *env) {
  (void) env;
  int arg = integer(first(args));
  if (arg == 0) return tee;
  else return nil;
...
object *uLisp::fn_oddp (object *args, object *env) {
  (void) env;
  int arg = integer(first(args));
...
object *uLisp::fn_evenp (object *args, object *env) {
  (void) env;
  int arg = integer(first(args));
...
object *uLisp::fn_char (object *args, object *env) {
  (void) env;
...
object *uLisp::fn_charcode (object *args, object *env) {
  (void) env;
  return number(fromchar(first(args)));
}

object *uLisp::fn_codechar (object *args, object *env) {
  (void) env;
  return character(integer(first(args)));
}

object *uLisp::fn_characterp (object *args, object *env) {
  (void) env;
...
object *uLisp::fn_stringp (object *args, object *env) {
  (void) env;
  return stringp(first(args)) ? tee : nil;
}

bool uLisp::stringcompare (object *args, bool lt, bool gt, bool eq) {
...
object *uLisp::fn_stringeq (object *args, object *env) {
  (void) env;
  return stringcompare(args, false, false, true) ? tee : nil;
}

object *uLisp::fn_stringless (object *args, object *env) {
  (void) env;
  return stringcompare(args, true, false, false) ? tee : nil;
}

object *uLisp::fn_stringgreater (object *args, object *env) {
  (void) env;
  return stringcompare(args, false, true, false) ? tee : nil;
}

object *uLisp::fn_sort (object *args, object *env) {
...
object *uLisp::fn_stringfn (object *args, object *env) {
  (void) env;
  object *arg = first(args);
  int type = arg->type;
...
object *uLisp::fn_concatenate (object *args, object *env) {
  (void) env;
  object *arg = first(args);
...
object *uLisp::fn_subseq (object *args, object *env) {
  (void) env;
  object *arg = first(args);
...
object *uLisp::fn_readfromstring (object *args, object *env) {   
  (void) env;
  object *arg = first(args);
...
void uLisp::pstr (char c) {
  buildstring(c, &GlobalStringIndex, &GlobalString);
}
 
object *uLisp::fn_princtostring (object *args, object *env) {   
  (void) env;
  object *arg = first(args);
  object *obj = myalloc();
  obj->type = STRING;
...
object *uLisp::fn_prin1tostring (object *args, object *env) {   
  (void) env;
  object *arg = first(args);
  object *obj = myalloc();
  obj->type = STRING;
...
object *uLisp::fn_logand (object *args, object *env) {
  (void) env;
...
object *uLisp::fn_logior (object *args, object *env) {
  (void) env;
...
object *uLisp::fn_logxor (object *args, object *env) {
  (void) env;
  unsigned int result = 0;
  while (args != NULL) {
    result = result ^ integer(first(args));
...
object *uLisp::fn_lognot (object *args, object *env) {
  (void) env;
  int result = integer(car(args));
  return number(~result);
}

object *uLisp::fn_ash (object *args, object *env) {
  (void) env;
...
object *uLisp::fn_logbitp (object *args, object *env) {
  (void) env;
...
object *uLisp::fn_eval (object *args, object *env) {
  return eval(first(args), env);
}

object *uLisp::fn_globals (object *args, object *env) {
  (void) args;
  if (GlobalEnv == NULL) return nil;
  return fn_mapcar(cons(symbol(CAR),cons(GlobalEnv,nil)), env);
}

object *uLisp::fn_locals (object *args, object *env) {
  (void) args;
  return env;
}

object *uLisp::fn_makunbound (object *args, object *env) {
  (void) env;
  object *key = first(args);
...
object *uLisp::fn_break (object *args, object *env) {
  (void) args;
...
object *uLisp::fn_read (object *args, object *env) {
  (void) args;
  (void) env;
  return read(gserial);
}

object *uLisp::fn_prin1 (object *args, object *env) {
  (void) env;
  object *obj = first(args);
...
object *uLisp::fn_print (object *args, object *env) {
  (void) env;
  pln(pserial);
  object *obj = first(args);
  printobject(obj, pserial);
...
object *uLisp::fn_princ (object *args, object *env) {
  (void) env;
  object *obj = first(args);
  char temp = PrintReadably;
  PrintReadably = 0;
...
object *uLisp::fn_terpri (object *args, object *env) {
  (void) args, (void) env;
  pln(pserial);
  return nil;
}

object *uLisp::fn_readbyte (object *args, object *env) {
  (void) env;
  return number((gstreamfun(args))());
}

object *uLisp::fn_readline (object *args, object *env) {
  (void) env;
  return readstring('\n', gstreamfun(args));
}

object *uLisp::fn_writebyte (object *args, object *env) {
  (void) env;
  int value = integer(first(args));
...
object *uLisp::fn_writestring (object *args, object *env) {
  (void) env;
  object *obj = first(args);
  char temp = PrintReadably;
  PrintReadably = 0;
...
object *uLisp::fn_writeline (object *args, object *env) {
  (void) env;
  object *obj = first(args);
  char temp = PrintReadably;
  PrintReadably = 0;
...
object *uLisp::fn_restarti2c (object *args, object *env) {
  (void) env;
...
object *uLisp::fn_gc (object *obj, object *env) {
...
object *uLisp::fn_room (object *args, object *env) {
  (void) args, (void) env;
  return number(Freespace);
}

object *uLisp::fn_saveimage (object *args, object *env) {
  if (args != NULL) args = eval(first(args), env);
  return number(saveimage(args));
}

object *uLisp::fn_loadimage (object *args, object *env) {
  (void) args, (void) env;
  return number(loadimage());
}

object *uLisp::fn_cls(object *args, object *env) {
  (void) args, (void) env;
  pserial(12);
  return nil;
}
...
object *uLisp::fn_pinmode (object *args, object *env) {
  (void) env;
...
object *uLisp::fn_digitalread (object *args, object *env) {
  (void) env;
  int pin = integer(first(args));
  if(digitalRead(pin) != 0) return tee; else return nil;
}

object *uLisp::fn_digitalwrite (object *args, object *env) {
  (void) env;
  int pin = integer(first(args));
  object *mode = second(args);
...
object *uLisp::fn_analogread (object *args, object *env) {
  (void) env;
  int pin = integer(first(args));
  checkanalogread(pin);
...
object *uLisp::fn_analogwrite (object *args, object *env) {
  (void) env;
  int pin = integer(first(args));
...
object *uLisp::fn_delay (object *args, object *env) {
  (void) env;
  object *arg1 = first(args);
  delay(integer(arg1));
  return arg1;
...
object *uLisp::fn_millis (object *args, object *env) {
  (void) args, (void) env;
  return number(millis());
}

object *uLisp::fn_note (object *args, object *env) {
  (void) env;
  static int pin = 255;
  if (args != NULL) {
...
object *uLisp::fn_edit (object *args, object *env) {
...
int uLisp::atomwidth (object *obj) {
...
void uLisp::superprint (object *form, int lm) {
...
void uLisp::supersub (object *form, int lm, int super) {
...
object *uLisp::fn_pprint (object *args, object *env) {
  (void) env;
  pln(pserial);
...
int uLisp::longsymbol (char *buffer) {
  char *p = SymbolTable;
  int i = 0;
...
void uLisp::deletesymbol (symbol_t name) {
...
void uLisp::testescape () {
  if (Serial.read() == '~') error(PSTR("Escape!"));
}

// Main evaluator

object *uLisp::eval (object *form, object *env) {
  int TC=0;
  EVAL:
  // Enough space?
...
void uLisp::pcharacter (char c, pfun_t pfun) {
...
void uLisp::pstring (char *s, pfun_t pfun) {
  while (*s) pfun(*s++);
}

void uLisp::printstring (object *form, pfun_t pfun) {
...
void uLisp::pfstring (PGM_P s, pfun_t pfun) {
  int p = (int)s;
  while (1) {
...
void uLisp::pint (int i, pfun_t pfun) {
  int lead = 0;
  if (i<0) pfun('-');
...
inline void uLisp::pln (pfun_t pfun) {
  pfun('\n');
}

void uLisp::pfl (pfun_t pfun) {
  if (LastPrint != '\n') pfun('\n');
}

void uLisp::printobject(object *form, pfun_t pfun){
...
object *uLisp::nextitem (gfun_t gfun) {
  char ch = gfun();
...
object *uLisp::readrest (gfun_t gfun) {
